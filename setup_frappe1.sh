...
install_wkhtmltopdf_deb () {
	...
	run_cmd wget http://download.gna.org/wkhtmltopdf/0.12/0.12.2.1/wkhtmltox-0.12.2.1_linux-prcise-amd64.deb-$WK_VER-$WK_ARCH.deb
...
setup_bench() {
	...
	run_cmd sudo su $FRAPPE_USER -c "cd /home/$FRAPPE_USER && bench init frappe-bench --frappe-branch v4.13.1"
    run_cmd sudo su $FRAPPE_USER -c "cd /home/$FRAPPE_USER/frappe-bench && bench get-app erpnext https://github.com/frappe/erpnext --branch v4.13.1"
	...
	run_cmd sudo bash -c "cd /home/$FRAPPE_USER/frappe-bench && bench setup sudoers $FRAPPE_USER"
	if $SETUP_PROD; then
		run_cmd sudo bash -c "cd /home/$FRAPPE_USER/frappe-bench && bench setup production $FRAPPE_USER"
	fi
}
